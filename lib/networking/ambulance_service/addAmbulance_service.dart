...
Future<MessageIdResponse> addAmbulance({AddAmbulanceRequest addAmbulanceRequest}) async {
  ...
  var res;
  try {
    res = await http.post(ENV.SERVER_IP + "/api/ambulance", body: requestData, headers: headers);
...
Future<ApiMessageResponse> addPublicAmbulance({PublicAmbulanceRequest publicAmbulanceRequest}) async {
  ...
  var res;
  try {
    res = await http.post(ENV.SERVER_IP + "/ambulance/requests", body: requestData, headers: headers);
  } on SocketException catch (e) {
    sendToast("There is a problem in internet");
    throw new SocketException(e.message);
  }
...
Future<MessageIdResponse> editAmbulance({AddAmbulanceRequest addAmbulanceRequest, String ambulanceId}) async {
  ...
  var res;
  try {
    res = await http.put(ENV.SERVER_IP + "/ambulance/$ambulanceId", body: requestData, headers: headers);
  } on SocketException catch (e) {
    sendToast("There is a problem in internet");
    throw new SocketException(e.message);
    // print(e);
...
Future<AddAmbulanceRequest> addAmbulanceList(
    {String name, String phone, String title, String division, String district, String thana}) async {
  print('Hi');
  ...
  print(ENV.SERVER_IP + "/ambulance?name=$name&phone=$phone&title=$title&division=$division&district=$district&thana=$thana");

  var res = await http.get(ENV.SERVER_IP + "/drugs?name=$name&phone=$phone&title=$title&division=$division&district=$district&thana=$thana",
      headers: headers);
...
Future<MessageResponseModel> deleteAmbulance({String ambulanceId}) async {
  ...
  var res = await http.delete(ENV.SERVER_IP + "/ambulance/$ambulanceId", headers: headers);
