...
Future<ApiMessageResponse> addPublicDrug(
    {PublicMedicineRequest publicMedicineRequest}) async {
  print('Hi');
...
Future<MessageIdResponse> editDrug(
    {AddDrugRequest addDrugRequest, String drugId}) async {
  print('Hi');
  print(addDrugRequest.name);
...
Future<DrugListResponse> getDrugList(
    {String name,
    String brand,
    String generic,
    int pageNo,
  ...
  var res = await http.get(
      "https://need-doctors-backend.herokuapp.com/drugs?pageNo=$pageNo&pageSize=$pageSize",
      headers: headers);
...
Future<List<String>> getGenericList({String genericName}) async {
  ...
  var res = await http.get("$SERVER_IP/drugs/generic?genericName=$genericName",
      headers: headers);
